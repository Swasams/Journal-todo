// ⚠️ DEV ONLY — remove this file (and references in stats_tab.dart) before
// shipping or pushing to a release branch. Used to populate fake data so the
// Stats tab is testable without weeks of real input.
...
class DevSeed {
  // Fill the app with ~60 days of plausible data. Idempotent: existing data
  // is overwritten so calling this again gives a clean fake slate.
  static Future<void> loadFakeData() async {
    final prefs = await SharedPreferences.getInstance();
    final rng = Random(42); // deterministic so tests are repeatable
    final now = DateTime.now();
    final days = List.generate(
        60, (i) => DateTime(now.year, now.month, now.day - (59 - i)));

    // ── Habits: keep current habit list, just populate completion log.
    ...
    for (final d in days) {
      ...
      for (final h in habits) {
        // ~70-90% completion rate, varies per habit
    ...
    // Bump completionCount on each habit to the number of days it appears.
    for (final h in habits) {
      var count = 0;
    ...
    // ── Metrics: realistic values per day.
    ...
    for (final m in metrics) {
      ...
      for (var i = 0; i < days.length; i++) {
        ...
        switch (m.name.toLowerCase()) {
          case 'weight':
            // 70.5 → 67.8, plus daily noise
            final base = 70.5 - 2.7 * progress;
            history[key] = (base + (rng.nextDouble() - 0.5) * 0.6)
                .toDouble();
            break;
          case 'mood':
            // 1..5, mostly 3-4
          ...
          case 'protein':
            ...
            break;
          default:
            // Generic numeric fill within target +- 25%
    ...
    // ── Journal: 10 entries with varied moods + rose/thorn/bud.
  ...
  // Wipe everything fake (and real). Resets seed flags so next launch
  ...
  static List<Map<String, String>> _journalSamples() => [
        ...
        },
        {
          ...
          'body':
              'Best kind of day. Tired in the right way.',
        },
        {
